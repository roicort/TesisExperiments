...
void randommining:: printCanonicalCode()
{
	...
	for(std::vector<canonicalcodeStruc*>::const_iterator it=code.begin(); it!=code.end();++it)
	{
		...
		//cout<<Lastestcancode<<endl;
			
	}
	/* get lexicographically smallest canonical code if it has more than one candidate */
	count=0;
	directioncode="";
	for(std::vector<canonicalcodeStruc*>::const_iterator it=code.begin(); it!=code.end();++it)
	{
		...
		for(std::vector<Edge*>::const_iterator itv=(*it)->edges.begin(); itv!=(*it)->edges.end();++itv )
		{
			
			if(myGraphSet[1]->biggraphvertex[(*itv)->st->label]->list.find(myGraphSet[1]->biggraphvertex[(*itv)->end->label])
			      !=myGraphSet[1]->biggraphvertex[(*itv)->st->label]->list.end())
			{
				if(myGraphSet[1]->biggraphvertex[(*itv)->end->label]->list.find(myGraphSet[1]->biggraphvertex[(*itv)->st->label])!=myGraphSet[1]->biggraphvertex[(*itv)->end->label]->list.end()){
					tempdirectioncode+="2,";
				}
				else{
					tempdirectioncode+="1,";//0 for out direction
				}
				
			}else{
				tempdirectioncode+="0,";//1 for in direction
		...
		if (count==0)
		{
			directioncode=tempdirectioncode;
			count++;
		}
		else{
			if(tempdirectioncode<directioncode)
			{
				directioncode=tempdirectioncode;
			}
		}	
	...
	Lastestcancode=Lastestcancode + "-"+ directioncode;	
	//cout<<Lastestcancode<<endl;
...
bool randommining::generateAnEdge(Vertex* endvertex, canonicalcodeStruc *it)
{
		...
		if(rawGraph->vertexmap.find((*itp)->label) == rawGraph->vertexmap.end()) continue;
			...
			et->st=endvertex;
			et->end=(*itp);
			//cout<<et->st->label<<" "<<et->end->label<<endl;
