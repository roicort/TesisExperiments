...
//		(*myGraphSet.begin())->printAdjacencyList();
		while(algoiteration++<noofIteration+1)
		{
			graphIndex =1;	
			gettimeofday(&start_time, (struct timezone *)0);
			st = start_time.tv_sec + (start_time.tv_usec/microsec);	
			...
			if(count[graphIndex]==false) {	
				...
				if(myGraphSet[graphIndex-1]->numV==sizeOfSubgrah)
				{
				 ...
				 if (algoiteration > noofIteration+1) break;
			...
			gettimeofday(&stop_time, (struct timezone *)0);
			if(algoiteration>1) sampleTime+=stop_time.tv_sec + (stop_time.tv_usec/microsec) -st;
			
			/* check to code gen  */
			gettimeofday(&start_time, (struct timezone *)0);
			st = start_time.tv_sec + (start_time.tv_usec/microsec);	
			if(algoiteration>1) getcanonicalCode();
			gettimeofday(&stop_time, (struct timezone *)0);
			if(algoiteration>1)  codeGenTime+=stop_time.tv_sec + (stop_time.tv_usec/microsec) -st;
			
			/*check to insert*/
			gettimeofday(&start_time, (struct timezone *)0);
			st = start_time.tv_sec + (start_time.tv_usec/microsec);
			if(algoiteration>1) insertIntoHeap();
			gettimeofday(&stop_time, (struct timezone *)0);
			if(algoiteration>1) insertHeapTime+=stop_time.tv_sec + (stop_time.tv_usec/microsec) -st;
		}
