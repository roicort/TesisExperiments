...
for file in tqdm(range(len(files))):
    ...
    savedf = pd.DataFrame()
    savedf['SourceIDS'] = auxdf['Source'].replace(nodesdict, inplace=False)
    savedf['TargetIDS'] = auxdf['Target'].replace(nodesdict, inplace=False)
    savedf.to_csv('graphdata/'+name+'.edges', index=False,sep=" ",header=False)
    os.system('clear')
    print('\n\n')
